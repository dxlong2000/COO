...
Answer the above question by following the steps below:
Step 1: Analyze the user's demographics and ideology one by one to infer their social and environmental values. Wrap this analysis by <EV> and </EV>.
Step 2: Analyze the user's historical opinions to infer their beliefs and norms from their social and environmental values. Wrap this analysis by <PBN> and </PBN>.
Step 3: From the above analyses, which opinion he is likely to choose?
...
"""   
    ...
    def __call__(self, implicit_persona: List[str], explicit_persona: List[str],
        ...
        try:
            ...
            generated_sent = "Answer: A"
        print("generated_sent: ", str(generated_sent))
        if "Answer: " in generated_sent: generated_sent = generated_sent.split("Answer: ")[1][0].strip()
        else: generated_sent = generated_sent[0]
        print("post generated_sent: ", str(generated_sent))
...
class PersonalizedQA:
    ...
    def mk_implicit_persona(self, option, persona_qa, random_qa):
        ...
        elif self.implicit_sampling == "chatgpttopk":
            rewritten_persona_questions = persona_qa['rewritten_persona_questions']
            gpt4_orders = persona_qa['chatgpt_orders']
            user_profile = [rewritten_persona_questions[gpt4_orders[idx]] for idx in range(12)]
        elif self.implicit_sampling == "reversedgpt4topk":
            # user_profile = [x for x, score in persona_qa["topk_opinions"].items()]
            rewritten_persona_questions = persona_qa['rewritten_persona_questions']
            gpt4_orders = persona_qa['gpt_4_orders']
            user_profile = [
                rewritten_persona_questions[gpt4_orders[idx]] for idx in range(8)
            ]
            user_profile.reverse()
        elif self.implicit_sampling == "gpt4topX":
            rewritten_persona_questions = persona_qa['rewritten_persona_questions']
            gpt4_orders = persona_qa['gpt_4_orders']
            gpt_4_selected_demonstration = persona_qa['gpt_4_selected_demonstration']
            user_profile = []
            for idx in range(len(gpt4_orders)):
                curr_idx = gpt4_orders[idx]
                if curr_idx in gpt_4_selected_demonstration:
                    user_profile.append(
                        rewritten_persona_questions[curr_idx]
                    )
        return utils.take(num=len(user_profile), arr=user_profile) if option == 0 or option == 2 else None
    ...
    def personalized_qa(self, persona: PersonaCreator, user_responses_jsons: Dict, option: int,
        ...
        for user_response_json in tqdm(user_responses_jsons, desc="users..."):
            ...
            for persona_qa in tqdm(limited_user_implicit_ques):
                    user_choice= "UNKNOWN"
                    if "chatgpt_orders" not in persona_qa: continue
                    if "chatgpt_attributes" not in persona_qa: continue
                    try:
                        all_explicit_traits = user_response_json["explicit_persona"]
                        all_traits_names = [list(ele.keys())[0] for ele in all_explicit_traits]

                        chatgpt_attributes = persona_qa["chatgpt_attributes"]
                        if len(chatgpt_attributes) > 0 and type(chatgpt_attributes[0]) == dict:
                            chatgpt_attributes = [list(ele.keys())[0] for ele in chatgpt_attributes]

                        selected_explicit_personae = [element for element in all_explicit_traits if list(element.keys())[0] in chatgpt_attributes]
                        print("selected_explicit_persona: ", str(selected_explicit_personae))
                        explicit_persona = self.mk_explicit_persona(all_traits=selected_explicit_personae) if option == 1 or option == 2 else None
...
if __name__ == '__main__':
    ...
    dir_name += f"-{args.implicit_sampling}-t{args.max_topics}-u{args.max_users}-q{args.max_ques}"
    if args.explicit_definition != "ideo_demo":
        dir_name += f"-explicit-means-{args.explicit_definition}"

    print(f"\nStart experiment: {dir_name} ...")

    persona = PersonaCreator(engine="gpt-3.5-turbo", openai_wrapper=OpenAIWrapper(cache_path=args.cache_path))
